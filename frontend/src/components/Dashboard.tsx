...
export function Dashboard({ address, balance, onRefresh, onSend, onReceive }: DashboardProps) {
  ...
  const balanceKsh = balance ? duffsToKsh(balance.balance) : '---';
    ...
    <div className="space-y-6">
      ...
      <div className="card text-center py-10">
        ...
        <h2 className="text-4xl font-bold tracking-tight">
          {balanceKsh}
          <span className="text-lg text-dark-400 ml-2">KSH</span>
        </h2>
        {balance && balance.received > 0 && (
          <p className="text-dark-500 text-sm mt-2">
            Total received: {duffsToKsh(balance.received)} KSH
          </p>
        )}
      </div>
      ...
      <div className="card">
        ...
        <button
          ...
          className="text-ksh-400 text-sm mt-2 hover:text-ksh-300 transition-colors"
        >
          Copy address
        </button>
      </div>

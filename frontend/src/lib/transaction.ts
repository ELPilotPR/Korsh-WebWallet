...
import { korshNetwork, COIN, DEFAULT_FEE_RATE, API_BASE } from './network';
...
export async function buildTransaction(params: {
  ...
  if (totalInput < amountDuffs + estimatedFee) {
    ...
    throw new Error(
      `Insufficient funds. Available: ${available.toFixed(8)} KSH, Needed: ${needed.toFixed(8)} KSH (including fee)`
    );
  }
  ...
  const keyPair = getECPair().fromPrivateKey(Buffer.from(privateKey), {
    network: korshNetwork,
  });

  const psbt = new Psbt({ network: korshNetwork });
...
// Format duffs to KSH display string
export function duffsToKsh(duffs: number): string {

...
import { korshNetwork, DERIVATION_PATH } from './network';
...
export function deriveFromMnemonic(mnemonic: string): {
  ...
  const root = getBip32().fromSeed(Buffer.from(seed), korshNetwork);
...
export function saveWallet(walletData: WalletData): void {
  localStorage.setItem('korsh_wallet', JSON.stringify(walletData));
}

// Load wallet from localStorage
export function loadWallet(): WalletData | null {
  const raw = localStorage.getItem('korsh_wallet');
  if (!raw) return null;
  try {
...
export function deleteWallet(): void {
  localStorage.removeItem('korsh_wallet');
}
